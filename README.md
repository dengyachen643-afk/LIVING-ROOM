...
每个成员都有一份只属于自己的互动轮次账本，私聊与群聊合并按真实发生顺序计数：

- 私聊中，一次成功的“Okra 消息 + 成员回复”算 1 轮。
- 群聊中，成员每次实际做出“回复”或“跳过”的决定算 1 轮；接口超时或报错不计入。
- 生成当前回复时，成员会拿到自己的最近 30 轮。它既包含私聊，也包含自己参与过的群聊，因此切换场景不会失忆。
- 每积累完整 30 轮（1–30、31–60……），服务端让该成员自己的模型在后台复盘这一批。聊天回复不用等待复盘完成。

复盘只提交少量原子候选，服务端再校验和落库：

| 类型 | 用途 | 生命周期 / 权限 |
| --- | --- | --- |
| 长期记忆 | 稳定偏好、关系信息、持续有用的事实 | 保留，属于该成员自己的 namespace |
| 短期记忆 | 近期状态、计划、正在延续的话题 | `hot` 约 3 天、`active` 约 14 天、`fading` 约 30 天，到期自然不再召回 |
| 共享记忆 | 五人都可以知道的公共事实 | 群聊事实可提交；私聊默认禁止，除非 Okra 明确允许公开 |
| 事件记忆 | 有时间、参与者和后续意义的节点 | 可私有或共享；共享事件同样需要公开证据 |

例如，Okra 在群里说“下个月准备搬家”，成员可以提交共享事实和事件；如果只在 Shin 私聊里说，默认只能进入 Shin 的私有长期/短期/事件记忆。模型不能自行把它扩散给其他人。

多个成员可能提交同一条共享记忆。服务端会用规范化正文、关键词和本地向量做 canonical merge，并记录 `confirmedBy` 与来源轮次；相近但互相冲突的事实不会静默覆盖。共享写入经过串行队列，因此并发复盘也不会轻易生成重复项。
...
1. 用本轮消息生成查询向量。
2. 只在当前成员 namespace 与 `shared` 中，从长期、未过期短期和事件记忆取候选，默认每类每个 namespace 最多取约 20 条。
3. 存储层结合关键词分数、向量余弦相似度和重要度做第一轮排序。
4. 提示词层再按完整短语、关键词命中、重要度和新旧程度轻量 rerank。
5. 按 ID、规范化正文和高向量相似度去重。
6. 控制短期与事件条数占比，避免临时状态挤掉稳定事实；最后限制总条数和总字符数。
...
最近 30 轮与从这些轮次刚提取的记忆会避免重复注入：原始上下文尚在时优先读原话，离开 30 轮窗口后再由 RAG 召回沉淀结果。这样既不遗漏第 1 轮的核心信息，也尽量不重复花 token。
...
- `state.json`：前端快速启动所需的最近 400 条消息、30 轮账本、长期/短期/事件记忆、头像与部分 UI 状态。
...
src/
  ...
  *-memory.js                     记忆整理动作生成与校验
  ...
  memory-consolidation.js         每 30 轮的分类复盘、权限校验与 canonical merge
  memory-review-coordinator.js    后台复盘调度、串行共享写入与失败重试入口
  member-rounds.js                成员轮次记录、跨场景上下文和历史回填
  memory-prompt.js                长期/短期/事件记忆的提示词标记
...
当前测试覆盖群聊并发与轮数、完整引用、重复回复、Kimi/GLM 请求格式、工具调用、记忆 CRUD、30 轮检查点、短期自然过期、共享权限与合并、统一向量检索、长期档案、主动推送、朋友圈任务、图片上传、断线恢复、前端草稿与移动端交互。
